...
`metaRange` uses an object orient programming paradigm, this means each species is described by its own biologically relevant processes and traits.
...
This can by done by using the `add_globals()` function for adding global variables and the `add_process()` function with no `species` argument specified, for adding global processes.
The global variables and processes are accessible through in the `globals`and `processes` field of the simulation object itself, respectively.
